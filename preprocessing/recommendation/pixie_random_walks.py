...
def generate_board_to_pin(userNum, paperNum):
    ...
    for userid in tqdm(range(userNum)):
        ...
        for i in range(tmp):
            board_to_pin[userid].append((random.randint(0, paperNum-1), math.ceil(34/(random.randint(0, 59) + 6))))
    return board_to_pin
...
def pixie_random_walk(board_to_pin, pin_to_board, cache_user_candidates, cache_paper_candidates, userid, chosen_paper_num, chosen_author_num, current_date, alpha=0.5, threshold_paper=10, threshold_author=80):
    ...
    count_author = dict()
    epoch = 0
    while len(chosen_paper) < chosen_paper_num or len(chosen_author) < chosen_author_num:
        # print(epoch, len(chosen_paper), len(chosen_author))
        epoch += 1
        curpaperid = random_walk_according_to_time(board_to_pin, userid, cache_user_candidates, current_date)
        while True:
            curuserid = random_walk_according_to_time(pin_to_board, curpaperid, cache_paper_candidates, current_date)
            if curuserid != userid:
                if curuserid in count_author:
                    count_author[curuserid] += 1
                    if count_author[curuserid] >= threshold_author and curuserid not in chosen_author:
                        chosen_author.add(curuserid)
                else:
                    count_author[curuserid] = 1

            curpaperid = random_walk_according_to_time(board_to_pin, curuserid, cache_user_candidates, current_date)
            if curpaperid not in userboard:
                if curpaperid in count_paper:
                    count_paper[curpaperid] += 1
                    if count_paper[curpaperid] >= threshold_paper and curpaperid not in chosen_paper:
    ...
    return sorted([(paperid, count_paper[paperid]) for paperid in chosen_paper], key=lambda x: x[1], reverse=True), sorted([(authorid, count_author[authorid]) for authorid in chosen_author], key=lambda x: x[1], reverse=True)

def pixie_random_walk_only_paper(board_to_pin, pin_to_board, cache_user_candidates, cache_paper_candidates, userid, chosen_paper_num, current_date, alpha=0.5, threshold_paper=10):
    userboard = set([tpl[0] for tpl in board_to_pin[userid]])
    chosen_paper = set()
    count_paper = dict()
    epoch = 0
    while len(chosen_paper) < chosen_paper_num :
        # print(epoch, len(chosen_paper))
        epoch += 1
        curpaperid = random_walk_according_to_time(board_to_pin, userid, cache_user_candidates, current_date)
        while True:
            curuserid = random_walk_according_to_time(pin_to_board, curpaperid, cache_paper_candidates, current_date)
    ...
    return sorted([(paperid, count_paper[paperid]) for paperid in chosen_paper], key=lambda x: x[1], reverse=True)

def pixie_random_walk_only_author(board_to_pin, pin_to_board, cache_user_candidates, cache_paper_candidates, userid, chosen_author_num, current_date, alpha=0.5, threshold_author=50, max_epoch=10000):
    userboard = set([tpl[0] for tpl in board_to_pin[userid]])
    chosen_author = set()
    count_author = dict()
    epoch = 0
    while len(chosen_author) < chosen_author_num and epoch < max_epoch:
        # print(epoch, len(chosen_author))
        epoch += 1
        curpaperid = random_walk_according_to_time(board_to_pin, userid, cache_user_candidates, current_date)
        while True:
            curuserid = random_walk_according_to_time(pin_to_board, curpaperid, cache_paper_candidates, current_date)
    ...
    return sorted([(authorid, count_author[authorid]) for authorid in chosen_author], key=lambda x: x[1], reverse=True)
...
if __name__ == "__main__":
    ...
    for userid in tqdm(range(userNum)):
        ...
        chosen_author = pixie_random_walk_only_author(board_to_pin, pin_to_board, cache_user_candidates, cache_paper_candidates, userid, 20, 60)
        # print(chosen_paper)
